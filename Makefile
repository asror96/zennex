...
composer-validate:
	docker compose -f docker/docker-compose.yml run --rm backend composer validate --strict

# Аудит пакетов Composer
composer-audit:
	docker compose -f docker/docker-compose.yml run --rm backend composer audit --format=plain
